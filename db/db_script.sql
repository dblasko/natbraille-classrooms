...
CREATE TABLE Notifications (
    ...
    userMail VARCHAR(50),
    link VARCHAR(128),
    CONSTRAINT fk_notifications_users FOREIGN KEY (userMail) REFERENCES Users(mail) ON DELETE CASCADE
);
...
CREATE TABLE Invites (
    ...
    CONSTRAINT fk_invites_promotionid FOREIGN KEY (promotionId) REFERENCES Promotions(id) ON DELETE CASCADE,
    CONSTRAINT fk_invites_invitedpersonmail FOREIGN KEY (invitedPersonMail) REFERENCES InvitedPerson(mail) ON DELETE CASCADE
);
...
CREATE TABLE PromotionMemberships (
    ...
    CONSTRAINT fk_promotionmemberships_promotionid FOREIGN KEY (promotionId) REFERENCES Promotions(id) ON DELETE CASCADE,
    CONSTRAINT fk_promotionmemberships_memberusermail FOREIGN KEY (memberUserMail) REFERENCES Users(mail) ON DELETE CASCADE,
...
CREATE TABLE Exercises (
    ...
    CONSTRAINT fk_exercise_createdbymail FOREIGN KEY (createdByMail) REFERENCES Users(mail) ON DELETE CASCADE
);
...
(
   ...
   CONSTRAINT fk_id FOREIGN KEY (idAffectation) REFERENCES EXERCISEAFFECTATION(id) ON DELETE CASCADE,
   CONSTRAINT fk_name FOREIGN KEY (userMail) REFERENCES Users(mail) ON DELETE CASCADE
);
...
(
   ...
   CONSTRAINT fk_idAttempt FOREIGN KEY (idAttempt) REFERENCES Attempt(id) ON DELETE CASCADE
);
...
(
   ...
   idExo                int(6),
   idPromotion  		int(6),
   CONSTRAINT fk_idExercise FOREIGN KEY (idExo) REFERENCES Exercises(id) ON DELETE CASCADE,
   CONSTRAINT fk_idPromotion FOREIGN KEY (idPromotion) REFERENCES Promotions(id) ON DELETE CASCADE
);
